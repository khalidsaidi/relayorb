...
function buildAiPrompt(params: {
  ...
  lines.push(
    [
      ...
      "You MUST leave no stone unturned, but you must not hallucinate.",
      "",
      "Workflow (research-first, do not stop early):",
      "PASS 1) Use the provided data below (OpenBB + StockPulse + FinnewsHunter) to form an initial view and list missing critical facts.",
      "PASS 2) If your environment supports browsing/opening links, research EVERY missing critical fact until either:",
      "  - you find an answer with a citation link (URL), OR",
      "  - you can justify why it is not currently knowable (paywalled, no public disclosure yet, etc.).",
      "PASS 3) Produce a concrete trade-plan template that uses ONLY verified levels/news and clearly states what would override the technical read.",
      "",
      "Hard rules:",
      "- For any fact not explicitly present in the provided data, include a citation link (URL).",
      "- If you cannot browse/open links, say so explicitly and do NOT guess. Instead provide: (a) a data-only analysis, and (b) a checklist of what to fetch next with exact queries/links.",
      "- If a value is unknown/unverifiable, write UNKNOWN and explain what source would confirm it.",
      "",
      ...
      "7) Verification checklist: what you looked up (with links) and what is still unknown.",
      "8) Trade plan template (NO position sizing / NO portfolio assumptions):",
      "   - Setup A (breakout/continuation): entry trigger, confirmation, invalidation, first target, management rules.",
      "   - Setup B (pullback/reclaim): entry trigger, confirmation, invalidation, target, management rules.",
      "   - Setup C (news catalyst): what counts as a real catalyst, what to ignore, and how it changes the technical levels.",
      "   - News that OVERRIDES technicals: a short list of headline types and exactly what you would re-check if they occur.",
      "   - A pre-trade checklist (5-10 bullets) based on what you verified.",
      "",
      `Ticker: ${symbol}`,
