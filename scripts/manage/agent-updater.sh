...
SERVICE_NAME=${SERVICE_NAME:-relayorb-agent-updater}
SERVICE_DIR="$ROOT_DIR/deploy/agent-updater"

usage() {
  cat <<USAGE
Usage: $(basename "$0") <command>

Note: This service runs on the bot-host VM via docker-compose, not Cloud Run.

Commands:
  build        Build container image via Cloud Build
  vm-logs      Tail recent updater logs on the bot host VM
  vm-restart   Restart updater container on the bot host VM
...
case "$command" in
  ...
  vm-logs)
    ...
    gcloud compute ssh "$bot_host_name" --zone "$bot_host_zone" \
      --command "sudo docker logs --tail 200 bot-host-relayorb-updater-1"
    ;;
  vm-restart)
    require_cmd gcloud
    gcloud compute ssh "$bot_host_name" --zone "$bot_host_zone" \
      --command "cd /opt/relayorb/deploy/bot-host && sudo docker compose up -d relayorb-updater"
    ;;
  local)
