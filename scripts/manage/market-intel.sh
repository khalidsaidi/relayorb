...
Env:
  ...
  MARKET_INTEL_SKIP_WHEN_CLOSED  true/false to skip runs when US market is closed
USAGE
}
...
build_env_vars() {
  ...
  if [ -n "${MARKET_INTEL_SKIP_WHEN_CLOSED:-}" ]; then
    envs="${envs}|MARKET_INTEL_SKIP_WHEN_CLOSED=${MARKET_INTEL_SKIP_WHEN_CLOSED}"
  fi
  echo "$envs"
}
