...
async function checkPriceSanity(items) {
  ...
  if (btc) {
    ...
    addCheck("sanity", "btc_price", "warning", "BTC price not found in price data")
  }
  ...
  if (eth) {
    ...
    if (price < CONFIG.sanityRanges.ethPrice.min || price > CONFIG.sanityRanges.ethPrice.max) {
      ...
      logVerbose(`ETH price $${price.toLocaleString()} is reasonable`)
    }
  }
  ...
  try {
    ...
    if (missingPrices > 0) {
      addCheck("consistency", "hot_trades_prices", "warning", 
        `${missingPrices}/${hotTrades.length} hot trades are missing price data`)
