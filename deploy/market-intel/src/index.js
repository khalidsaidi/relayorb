...
async function readControls(db) {
  ...
  const forexEnabled = data?.forexEnabled !== false
  if (config.pipelineEventsEnabled) {
    await publishPipelineEvent(
      buildPipelineEvent({
        stationId: "market_intel",
  ...
  const assetFocus =
    ...
    []
  const filteredFocus = assetFocus.filter((asset) => {
    if (asset === "crypto") return cryptoEnabled
    if (asset === "forex") return forexEnabled
    return true
  })
  const defaultFocus = ["crypto", "stock", "forex"].filter((asset) => {
    if (asset === "crypto") return cryptoEnabled
    if (asset === "forex") return forexEnabled
    return true
  })
  const normalizedFocus = filteredFocus.length > 0 ? filteredFocus : defaultFocus
...
function buildFetchStatus({ items, error, preferences, listKey, source, disabled }) {
  if (disabled) {
    return {
      status: "disabled",
      count: 0,
...
async function run() {
  ...
  const cryptoEnabled = controls.cryptoEnabled !== false
  const forexEnabled = controls.forexEnabled !== false
  const focusInput = Array.isArray(controls.assetFocus)
    ? controls.assetFocus
    : ["crypto", "stock", "forex"]
  const focusFiltered = focusInput.filter((asset) => {
    if (asset === "crypto") return cryptoEnabled
    if (asset === "forex") return forexEnabled
    return true
  })
  controls.assetFocus = focusFiltered.length > 0 ? focusFiltered : ["stock"]
  const primaryAssets = controls.primaryAssets || { crypto: [], stocks: [], forex: [] }
  controls.primaryAssets = {
    crypto: cryptoEnabled ? primaryAssets.crypto || [] : [],
    stocks: primaryAssets.stocks || [],
    forex: forexEnabled ? primaryAssets.forex || [] : [],
  }
  ...
  const [cryptoResult, stockResult, forexResult, accuracySummary, botRegistryResult] =
    await Promise.all([
      cryptoEnabled
        ...
        : Promise.resolve({ items: [], error: null, source: "disabled" }),
  ...
  const isStatusOk = (status, allowEmpty = false) => {
    if (status === "ok" || status === "disabled") return true
    if (allowEmpty && status === "empty") return true
    return false
  }
  const stockOk = isStatusOk(fetchStatus.stock.status, !stockMarketOpen)
  const baseStatus =
    isStatusOk(fetchStatus.crypto.status) &&
    stockOk &&
    isStatusOk(fetchStatus.forex.status)
      ? "ok"
      : "degraded"
